...
@napari_hook_specification(firstresult=True)
def napari_get_writer(path: str, layer_types: List[str]) -> Optional[WriterFunction]:
...
@napari_hook_specification(historic=True)
def napari_experimental_provide_function() -> Union[FunctionType, List[FunctionType]]:
...
def napari_experimental_provide_theme() -> Dict[
    ...
    See :class:`~napari.utils.theme.Theme` for more detail of what are the required
    keys.
